...
def read_dataset(dataset_path_AD, dataset_path_CTRL, x_id ="AD-", y_id="CTRL-"):
    ...
    """
    file_names_AD = glob(os.path.join(dataset_path_AD, f"*{x_id}*.nii"  ))
    file_names_CTRL= glob(os.path.join(dataset_path_CTRL, f"*{y_id}*.nii"  ))
    X = []
    Y = []
    ...
    for fname_CTRL in file_names_CTRL:
        ...
        Y.append(0)
    return np.array(X), np.array(Y), file_names_AD, file_names_CTRL
